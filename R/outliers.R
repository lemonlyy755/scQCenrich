...
.qc_feature_matrix <- function(metrics) {
  ...
  feats <- intersect(
    c(
      "pctMT", "nFeature", "stress_score", "MALAT1_frac",
...
detect_outliers <- function(metrics,
  ...
  zfix <- function(v) {
    ...
    out <- (v - mean(v, na.rm = TRUE)) / s
    bad <- !is.finite(out)
    if (any(bad) && isTRUE(getOption("scQCenrich.debug", FALSE))) {
      message(sprintf("[detect_outliers][zfix] non-finite z replaced with 0: %d / %d", sum(bad), length(out)))
    }
    out[bad] <- 0
    out
  }
  ...
  sdz <- function(v) {
    ...
    bad <- !is.finite(out)
    if (any(bad) && isTRUE(getOption("scQCenrich.debug", FALSE))) {
      message(sprintf("[detect_outliers][sdz] non-finite robust z replaced with 0: %d / %d", sum(bad), length(out)))
    }
    out[bad] <- 0
    out
  }
  ...
  rZ <- function(x) { # robust z
    ...
    bad <- !is.finite(out)
    if (any(bad) && isTRUE(getOption("scQCenrich.debug", FALSE))) {
      message(sprintf("[detect_outliers][rZ] non-finite robust z replaced with 0: %d / %d", sum(bad), length(out)))
    }
    out[bad] <- 0
    out
  }
  ...
  flag_extreme_pctmt <- function(df, pct, nfeat, sample) {
    ...
    if (is.null(pct)) {
      return(list(flag = rep(FALSE, nrow(df)), reason = rep("", nrow(df))))
    }
  ...
  qc_embed <- function(df, n_pcs = 5) {
    ...
    # Add two-tailed deviation as a dedicated axis (strong signal)
    # When intronic_frac is present use it; otherwise MALAT1 acts as proxy
    has_intronic <- "intronic_frac" %in% colnames(df) && !all(is.na(df$intronic_frac))
    has_malat <- "MALAT1_frac" %in% colnames(df) && !all(is.na(df$MALAT1_frac))

    if (has_intronic) {
      intr_dev <- rZabs(df$intronic_frac)
      X <- cbind(X, intr_dev)
      colnames(X)[ncol(X)] <- "intr_dev"
      # MALAT1 added as a lightweight secondary signal only when splicing present
      if (has_malat) {
        X <- cbind(X, zfix(df$MALAT1_frac))
        colnames(X)[ncol(X)] <- "malat"
      }
    } else if (has_malat) {
      # No splicing data: MALAT1 two-tailed deviation substitutes for intronic dev
      malat_dev <- rZabs(df$MALAT1_frac)
      X <- cbind(X, malat_dev)
      colnames(X)[ncol(X)] <- "malat_proxy_dev"
    }

    kp <- min(n_pcs, ncol(X))
    if (kp <= 1L) {
      return(matrix(X[, 1L], ncol = 1L, dimnames = list(rownames(df), "PC1")))
    }
  ...
  cl <- switch(method,
    gmm = {
      ...
      out <- tryCatch(
        {
          mns$Mclust(emb, G = 1:6)
        },
        error = function(e) {
          if (grepl("mclustBIC", conditionMessage(e), ignore.case = TRUE)) {
            used_fallback <<- TRUE
            NULL
          } else {
            stop(e)
  ...
  # When splicing data is absent, MALAT1 two-tailed deviation acts as proxy
  intronic_available <- "intronic_frac" %in% colnames(metrics) &&
    !all(is.na(metrics$intronic_frac))
  malat_available <- "MALAT1_frac" %in% colnames(metrics) &&
    !all(is.na(metrics$MALAT1_frac))

  intr_dev_cell <- if (intronic_available) {
    rZabs(metrics$intronic_frac)
  } else if (malat_available) {
    # MALAT1 proxy: high nuclear fraction can signal damaged/partial cells
    rZabs(metrics$MALAT1_frac)
  } else {
    rep(0, nrow(metrics))
  }
  ...
  zdf <- data.frame(
    ...
    strs = sdz(agg$stress),
    # intr = w_intr weight applied to either true intronic dev OR MALAT1 proxy dev
    intr = w_intr * sdz(agg$intrDev),
    u2s  = w_u2s * sdz(agg$u2s),
    # Suppress the separate MALAT1 term when it is already driving intrDev (proxy)
    # to avoid double-counting the same signal
    mal  = if (!intronic_available || all(is.na(agg$malat))) 0 else 0.5 * sdz(agg$malat)
  )
  ...
  if (getOption("scQCenrich.debug", FALSE)) {
    msg <- sprintf(
      "[detect_outliers] worst=%s | remove=%d borderline=%d keep=%d | thr=%.3f border_z=%.2f | w_intr=%.2f intr_border=%.2f intr_escal=%.2f",
