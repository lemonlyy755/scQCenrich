utils::globalVariables(c("UMAP_1", "UMAP_2", "qc_status", "is_doublet", "group", "rate", "val", "is_low_quality", "dbl_method", "dbl_score", "rescue_flag", "rescue_reason", "pANN", "DF.classifications", "scDblFinder.class", "scDblFinder.score"))
...
#'   \code{$status_df}, \code{$report_file}.
...
run_qc_pipeline <- function(obj,
                            ...
                            doublets = c(
                              "none", "auto", "doubletfinder", "scdblfinder"
                            ),
                            remove_doublets = TRUE,
                            sample_col = NULL,
                            annot_method = c(
                              "marker_score", "singleR", "sctype", "none"
                            ),
                            ...
                            debug = FALSE,
                            sctype_db_path = system.file(
                              "extdata", "ScTypeDB_full.xlsx",
  ...
  if (identical(sctype_db_path, "") || !file.exists(sctype_db_path)) {
    if (isTRUE(debug)) {
      message("[scQCenrich-debug] No packaged ScType DB found; using NULL.")
    }
    sctype_db_path <- NULL
  }
    ...
    if (am == "singler") {
      ...
      if (marker_source == "panglao") {
        ...
        if (!nzchar(panglao_file) ||
          ...
          stop(
            "Panglao file not found. Provide a valid path via `panglao_file`."
          )
        }
        ...
        stop(
          "marker_source='custom' requires passing custom_signatures ",
          "to auto_annotate()."
        )
      }
  ...
  # [2/7] Assay/layer prep (ensure non-empty 'data' & v5 layers joined)
  Seurat::DefaultAssay(obj) <- assay
  obj <- .ensure_layers_and_data(obj, assay = assay)
  ...
  flags <- flagLowQuality(
    ...
    obj = obj,
    celltype_col = if ("auto_celltype" %in% colnames(obj@meta.data)) {
      "auto_celltype"
    } else {
      NULL
    },
  ...
  if (getOption("scQCenrich.debug", FALSE)) {
    ...
    utils::write.csv(
      status, file.path(outdir, "qc_status.csv"),
      row.names = TRUE
    )
    ...
    if (!is.null(gt)) {
      utils::write.csv(
        gt, file.path(outdir, "rescue_gate_table.csv"),
        row.names = FALSE
      )
    }

    message(sprintf(
      "[subset] keep=%d | borderline=%d | remove=%d",
  ...
  if (isTRUE(getOption("scQCenrich.debug", FALSE))) {
    ...
    utils::write.csv(
      status, file.path(save_dir, "qc_status.csv"),
      row.names = TRUE
    )
    ...
    if (!is.null(gt)) {
      utils::write.csv(
        gt, file.path(save_dir, "rescue_gate_table.csv"),
        row.names = FALSE
      )
    }
    ...
    if (!is.null(gt_alt)) {
      utils::write.csv(
        gt_alt, file.path(save_dir, "rescue_gate_table_lenient.csv"),
        row.names = FALSE
      )
    }
    if (!is.null(attr(flags, "rescue_alt"))) {
      writeLines(
        names(which(attr(flags, "rescue_alt")$rescue_flag)),
  ...
  if (is.null(rownames(det))) {
    stop("QC table rownames are NULL (expected cell barcodes).")
  }

  keep_cells <- intersect(
    colnames(obj), rownames(det)[det$qc_status %in% c("keep", "borderline")]
  )

  if (getOption("scQCenrich.debug", FALSE)) {
    message(sprintf(
      "[subset] keep=%d | borderline=%d | remove=%d | in_obj=%d | selected=%d",
    ...
    if (length(keep_cells)) {
      message(sprintf(
        "[subset] example kept: %s",
        paste(head(keep_cells, 5), collapse = ", ")
      ))
    }
  }
  ...
  if (length(keep_cells) == 0L) {
    utils::write.csv(
      det, file.path(save_dir, "qc_status.csv"),
      row.names = TRUE
    )
    stop(
      "QC produced zero keep/borderline cells. See: ",
      file.path(save_dir, "qc_status.csv")
    )
  }

  obj_keep <- subset(obj, cells = keep_cells)

  rep_path <- if (dirname(report_file) %in% c(".", "")) {
    file.path(save_dir, report_file)
  } else {
    report_file
  }
  if (isTRUE(report_html)) {
    ...
    if (!("qc_status" %in% colnames(obj@meta.data))) {
      obj@meta.data$qc_status <- factor(
        "keep",
...
detect_doublets_doubletfinder <- function(obj,
  ...
  odef <- Seurat::DefaultAssay(obj)
  on.exit(Seurat::DefaultAssay(obj) <- odef, add = TRUE)
  Seurat::DefaultAssay(obj) <- assay
  ...
  sweep_list <- DoubletFinder::paramSweep(obj, PCs = 1:20, sct = FALSE)
...
detect_doublets_scdblfinder <- function(obj, assay = Seurat::DefaultAssay(obj)) {
  stopifnot(
