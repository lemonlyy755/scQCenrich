...
rescue_by_coherence <- function(
  ...
  # ---- choose grouping column (always use QC cluster)
  ...
  # ---- Intronic Baseline Logic (extract intronic if available)
  intr <- if ("intronic_frac" %in% colnames(met)) met$intronic_frac else NULL
  global_med_intr <- get_med(intr)
  ...
  med_by <- function(v) {
    if (is.null(v)) {
      return(rep(NA_real_, length(unique(cl))))
    }
    tapply(v, cl, function(x) stats::median(x, na.rm = TRUE))
  }

  cluster_ids <- as.character(names(tapply(cl, cl, length)))
  cm <- data.frame(
    cluster = cluster_ids,
  ...
  # Inject intronic medians if we have that coverage
  if (!is.null(intr)) {
    cm$med_intr <- as.numeric(med_by(intr))
  } else {
    cm$med_intr <- rep(NA_real_, nrow(cm))
  }
  ...
  # Calculate base score (with removal penalty)
  ...
  # A cluster must be appropriately sized, score past the threshold, and NOT be mathematically comprised of ruptured naked nuclei
  cm$eligible <- cm$ok_size & (cm$score >= resc_threshold) & cm$pass_intr_healthy
  ...
  gate_tbl <- cm[order(-cm$removed), c(
    "cluster", "size", "removed", "removed_frac", "med_mito", "med_feat", "med_stress", "med_intr",
    "coherence", "ok_size", "pass_mito", "pass_feat", "pass_strs", "n_pass", "n_gates",
    "pass_frac", "score", "pass_intr_healthy", "pass_coh", "eligible"
  )]
