...
If you pass an unrecognised tissue name, `run_qc_pipeline()` warns and
annotation falls back to all tissues, which is less specific. Use
`list_panglao_tissues()` to see the supported PanglaoDB tissue names:
...
If you wish to compare `scQCenrich`’s performance against standard
